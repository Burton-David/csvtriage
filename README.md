...
```python
try:
    frame = ct.read("data.csv")   # a row that recovery could not place
except ct.ParseError as exc:
    print(exc)
...
name Python has no codec for), and `ParseError`. A missing file raises the builtin
`FileNotFoundError`.
...
3.10–3.13. On the roadmap: numeric/date normalization, recovery policies,
CleverCSV-grade dialect detection, and optional validation.
